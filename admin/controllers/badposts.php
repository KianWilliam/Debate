...
class DebateControllerBadposts extends AdminController
{
	public function getModel($name = 'Badpost', $prefix = 'DebateModel', $config = array('ignore_request' => true))
	{
		$model = parent::getModel($name, $prefix, $config);
		
		return $model;
	}
	public function deleteBadUsers()
	{
    ...
    else
    {
		...
		$query = $db->getQuery(true);
			$query->select('messageid')->from($db->quoteName('#__debate_badpost_users'))->where($db->quoteName('id'). ' = ' . (int) $value);
             $db->setQuery($query);
