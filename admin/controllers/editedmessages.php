...
class DebateControllerEditedmessages extends AdminController
{
	public function getModel($name = 'Editedmessages', $prefix = 'DebateModel', $config = array('ignore_request' => true))
	{
		$model = parent::getModel($name, $prefix, $config);
		
		return $model;
	}
	public function deleteEditedMessages()
	{
    ...
    else
    {
		...
		$query = $db->getQuery(true);
			$query->select('messageid')->from($db->quoteName('#__debate_edit_users'))->where($db->quoteName('id'). ' = ' . (int) $value);
             $db->setQuery($query);
