...
class DebateModelEditedmessages extends ListModel {
	...
	public function __construct($config = array()) {
	  ...
	  protected function getListQuery() {
        ...
        $query->select('*');
        $query->from('#__debate_edit_users AS a');
       ...
       $filteruserid = $this->getState('filter.userid', null);
        ...
        {
				$query->where('a.editdate LIKE ' . $db->quote('%'.$db->escape(trim($filterdate), true).'%')  );
   ...
   try {
        $query = $db->getQuery(true)
                    ->delete($db->quoteName("#__debate_edit_users"))
                    ->where("id IN ( ".$cid.")");
					//var_dump($query);
					//exit();
        $db->setQuery($query);
