...
<form action="<?php echo Route::_('index.php?option=com_debate&view=deprivelistusers'); ?>" method="post" name="adminForm" id="adminForm">
		...
		<tbody>
            ...
            ?>
					<tr class="row<?php echo $i % 2; ?>">
							<td class="left">
                               <?php echo HTMLHelper::_('grid.id', $i, $depriveuserID); ?>
                            </td>
							<td style="text-align:center">
                                <a href="<?php echo Route::_('index.php?option=com_debate&task=deprivelistuser.edit&id='.(int) $depriveuser->id); ?>"><?php echo $depriveuser->id; ?></a>
							...
							<td class="order" style="text-align:center">
                               ...
                               <?php if ($saveOrder) : ?>
                                <?php if ($listDirn == 'asc') : ?>
                                    <span><?php echo $this->pagination->orderUpIcon($i, ($depriveuser->ordering == @$this->items[$i - 1]->ordering), 'deprivelistusers.orderup', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
                                    <span><?php echo $this->pagination->orderDownIcon($i, $n, ($depriveuser->ordering == @$this->items[$i + 1]->ordering), 'deprivelistusers.orderdown', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
                                <?php elseif ($listDirn == 'desc') : ?>
                                    <span><?php echo $this->pagination->orderUpIcon($i, ($depriveuser->ordering == @$this->items[$i - 1]->ordering), 'deprivelistusers.orderdown', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
                                    <span><?php echo $this->pagination->orderDownIcon($i, $n, ($depriveuser->ordering == @$this->items[$i + 1]->ordering), 'deprivelistusers.orderup', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
                                <?php endif; ?>
                               <?php endif; ?>
