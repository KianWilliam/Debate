...
Joomla.submitbutton = function(task)
	{
		if (task == 'depriveusers.deleteUsers') {
			if(confirm("Are you sure to delete these record(s)? the related original messages shall be updated too!")){
				...
				if (task === 'depriveuser.edit' || task ==='depriveusers.publish' || taks==='depriveusers.unpublish' ) {
										Joomla.submitform(task, document.getElementById('adminForm'));
...
<form action="<?php echo Route::_('index.php?option=com_debate&view=depriveusers'); ?>" method="post" name="adminForm" id="adminForm">
	...
	<table class="adminlist">
		<thead>
		    <tr>
				...
				<th width="1%">
                    <?php echo HTMLHelper::_('grid.sort', 'JGRID_HEADING_DEPRIVE_DATE', 'a.deprivedate', $listDirn, $listOrder); ?>
                </th>
            </tr>
	    </thead>
		<tfoot>
		...
		<tbody>
            ...
            ?>
					<tr class="row<?php echo $i % 2; ?>">
							<td class="left">
                               <?php echo HTMLHelper::_('grid.id', $i, $depriveuser->id); ?>
                            </td>
							<td style="text-align:center">
                                <a href="<?php echo Route::_('index.php?option=com_debate&task=depriveuser.edit&id='.(int) $depriveuser->id); ?>"><?php echo $depriveuser->id; ?></a>
							...
							<td class="order" style="text-align:center">
                               ...
                               <?php if ($saveOrder) : ?>
                                <?php if ($listDirn == 'asc') : ?>
                                    <span><?php echo $this->pagination->orderUpIcon($i, ($depriveuser->ordering == @$this->items[$i - 1]->ordering), 'depriveusers.orderup', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
                                    <span><?php echo $this->pagination->orderDownIcon($i, $n, ($depriveuser->ordering == @$this->items[$i + 1]->ordering), 'depriveusers.orderdown', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
                                <?php elseif ($listDirn == 'desc') : ?>
                                    <span><?php echo $this->pagination->orderUpIcon($i, ($depriveuser->ordering == @$this->items[$i - 1]->ordering), 'depriveusers.orderdown', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
                                    <span><?php echo $this->pagination->orderDownIcon($i, $n, ($depriveuser->ordering == @$this->items[$i + 1]->ordering), 'depriveusers.orderup', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
                                <?php endif; ?>
                               <?php endif; ?>
                               <?php 
