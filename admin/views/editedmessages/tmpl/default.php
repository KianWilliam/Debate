...
Joomla.submitbutton = function(task)
	{
		if (task == 'editedmessages.deleteEditedMessages') {
...
<form action="<?php echo Route::_('index.php?option=com_debate&view=editedmessages'); ?>" method="post" name="adminForm" id="adminForm">
	...
	<table class="adminlist">
		<thead>
		    <tr>
				...
				<th width="1%">
                    <?php echo HTMLHelper::_('grid.sort', 'JGRID_HEADING_EDIT_DATE', 'a.editdate', $listDirn, $listOrder); ?>
                </th>
            </tr>
	    </thead>
		<tfoot>
		...
		<tbody>
			<?php
                    $n = count($this->items);
                    foreach ($this->items as $i => $editedmessage) :
                        ...
                        $userid = $this->escape($editedmessage->userid);
						
            ?>
					<tr class="row<?php echo $i % 2; ?>">
							<td class="left">
                               <?php echo HTMLHelper::_('grid.id', $i, $editedmessageID); ?>
                            </td>
							<td style="width:1%; text-align:center">
                                <?php 
								
								echo $editedmessageID;
								?>	
                            </td>
							...
							<td class="order" style="text-align:center">
                               ...
                               <?php if ($saveOrder) : ?>
                                <?php if ($listDirn == 'asc') : ?>
                                    <span><?php echo $this->pagination->orderUpIcon($i, ($editedmessage->ordering == @$this->items[$i - 1]->ordering), 'editedmessages.orderup', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
                                    <span><?php echo $this->pagination->orderDownIcon($i, $n, ($editedmessage->ordering == @$this->items[$i + 1]->ordering), 'editedmessages.orderdown', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
                                <?php elseif ($listDirn == 'desc') : ?>
                                    <span><?php echo $this->pagination->orderUpIcon($i, ($editedmessage->ordering == @$this->items[$i - 1]->ordering), 'editedmessages.orderdown', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
                                    <span><?php echo $this->pagination->orderDownIcon($i, $n, ($editedmessage->ordering == @$this->items[$i + 1]->ordering), 'editedmessages.orderup', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
                                <?php endif; ?>
                               <?php endif; ?>
                               <?php $disabled = $saveOrder ? '' : 'disabled="disabled"'; ?>
                                <input  type="text" name="order[]" size="5" value="<?php echo $editedmessage->ordering; ?>" <?php echo $disabled ?> class="text-area-order" />
                             <?php else : ?>
                                <?php echo $editedmessage->ordering; ?>
                             <?php endif; ?>
                           </td>
						   <td align="center">
                              <?php echo $userid; ?>
						    ...
						    <td align="center">
                              <?php 
								  echo $editedmessage->fonderid; 				
							  ?>
                           </td>
						    <td align="center">
                              <?php 
								  echo $editedmessage->reason; 				
							  ?>
							  <hr />
                           </td>
						    <td align="center">
                              <?php 
								  echo $editedmessage->messageid; 				
							  ?>
						    ...
						    <td align="center">
                              <?php 
								  echo Date($editedmessage->editdate); 				
							  ?>
                           </td>
					</tr>
					<?php endforeach; ?>
